...
// Get rentable by query
rentableRoute.route('/getRA/:name').get((req, res) => {
    Rentable.find(req.params, req.body, (error, data) => {
        if (error) {
            return next(error)
        } else {
            res.json(data)
...
rentableRoute.route('/deleteRA/:id').delete((req, res, next) => {
    Rentable.findByIdAndRemove(req.params.id, (error, data) => {
        if (error) {
            return next(error);
        } else {
            res.status(200).json({
